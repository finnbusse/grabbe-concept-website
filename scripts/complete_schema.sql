...
-- ============================================================================
-- 10. TAGS TABLE
-- Purpose: Tags for categorizing events, documents, and posts
-- ============================================================================
...
CREATE POLICY "event_tags_select_all" ON public.event_tags FOR SELECT USING (true);
CREATE POLICY "event_tags_insert_auth" ON public.event_tags FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "event_tags_delete_auth" ON public.event_tags FOR DELETE USING (auth.uid() IS NOT NULL);

-- 12. DOCUMENT_TAGS JUNCTION TABLE
...
CREATE POLICY "document_tags_select_all" ON public.document_tags FOR SELECT USING (true);
CREATE POLICY "document_tags_insert_auth" ON public.document_tags FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY "document_tags_delete_auth" ON public.document_tags FOR DELETE USING (auth.uid() IS NOT NULL);

-- 13. POST_TAGS JUNCTION TABLE
