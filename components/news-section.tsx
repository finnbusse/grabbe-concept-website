            ...
            <Link href={`/aktuelles/${featured.slug}`} className="group relative flex flex-col overflow-hidden rounded-2xl border border-border/60 bg-card/80 backdrop-blur-sm transition-all duration-500 hover:shadow-xl hover:shadow-primary/[0.06] hover:border-primary/30 h-full">
              {featured.image_url ? (
                <div className="relative h-72 overflow-hidden">
                  <Image src={featured.image_url} alt={featured.title} fill sizes="(max-width: 768px) 100vw, 60vw" className="object-cover transition-transform duration-700 group-hover:scale-105" />
                  ...
                  {(featured.author_name || featured.author_profile) && (
                    <span className="flex items-center gap-1.5">
                      {featured.author_profile?.avatar_url ? (
                        <Image src={featured.author_profile.avatar_url} alt="" width={20} height={20} className="rounded-full object-cover" />
                      ) : (
                ...
                >
                  {post.image_url ? (
                    <div className="h-24 w-24 shrink-0 overflow-hidden rounded-xl">
                      <Image src={post.image_url} alt={post.title} fill sizes="(max-width: 768px) 100vw, 30vw" className="object-cover transition-transform duration-500 group-hover:scale-110" />
                    </div>
                  ) : (
                      ...
                      {(post.author_name || post.author_profile) && (
                        <span className="flex items-center gap-1">
                          {post.author_profile?.avatar_url ? (
                            <Image src={post.author_profile.avatar_url} alt="" width={16} height={16} className="rounded-full object-cover" />
                          ) : (
