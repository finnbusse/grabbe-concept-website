...
function getButtonClass(style: CampaignButton["style"], accentColor: string) {
  switch (style) {
    case "primary":
      return "text-white"
    case "secondary":
            ...
            {activeCampaign.buttons.map((btn) => {
              ...
              return (
                <a
                  ...
                  className={`inline-flex items-center justify-center rounded-lg px-5 py-2.5 text-sm font-medium transition-colors ${getButtonClass(btn.style, accentColor)}`}
                  style={{
                    backgroundColor: isPrimary ? accentColor : undefined,
                    borderColor: isOutline ? accentColor : undefined,
                    color: isOutline ? accentColor : undefined,
                  }}
                  onClick={handleClose}
                >
                  {btn.label}
